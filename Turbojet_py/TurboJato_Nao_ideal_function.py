...
def turboJato_Nao_Ideal(type):
    ...
    MFP_c = m.sqrt(gamma_c/R_t) * 0.7 * (1 + (gamma_c - 1)/2 * 0.7**2)**((gamma_c + 1)/2/(gamma_c - 1)) #considerando M_c = 0.7  (?) e gamma_c e R_t (?)
    T5 = Tt4 * tau_t #K
    P5 = P0 * pi_c * pi_t #K
    ...
    return [M0,T0,P0,tau_r,pi_r,Tt4,pi_d,pi_c,tau_c,Pt9_P9]
