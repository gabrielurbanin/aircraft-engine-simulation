...
if tipo == 1:
    ...
    # Extra inputs from rolls royce nene
...
if tipo == 2:
    m0 = 80/m.sqrt(gamma * R * (650 + 273.15)); #kg/s (?) VT80

F = F_m0 * m0  #N
FC = S * F * 60 #(kgFuel/min) seria mf (?)
MFP_c = m.sqrt(gamma/R) * 0.7 * (1 + (gamma-1)/2 * 0.7**2)**((gamma + 1)/2/(gamma - 1)) #considerando M_c = 0.7  (?)
T5 = Tt4 * tau_t #K
